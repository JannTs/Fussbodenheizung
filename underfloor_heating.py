...
def select_data_range_by_month(df):
    ...
    if not month_map:
        print("Не удалось построить карту месяцев. Будут использованы все данные.")
        return 1, len(df)

    print("\n📅 Обнаружены следующие месяцы в данных:")
    for month, (start, end) in month_map.items():
        print(f"Месяц {month:02d}: строки {start} — {end}")
    
    while True:
        user_input = input(
            "\nВведите номер месяца (1–12), строку начала (в пределах одного месяца), "
            "или диапазон в формате start-end: ").strip()

        if user_input.isdigit():
            month_num = int(user_input)
            if month_num in month_map:
                start, end = month_map[month_num]
                print(f"Выбран месяц {month_num:02d}, строки {start} — {end}")
                return start, end
            else:
            ...
            try:
                ...
                if start >= end:
                    print("❌ Начальная строка должна быть меньше конечной.")
                    continue
                ...
                if month_start != month_end:
                    print("❌ Диапазон должен относиться к одному месяцу.")
                    continue

                print(f"Выбран диапазон строк: {start} — {end}")
                return start, end

            except Exception:
                print("❌ Неверный формат диапазона. Пример: 3500-4500")

        else:
            try:
                start = int(user_input)
                if start < 1 or start > df.index[-1] + 1:
                    print("❌ Номер строки вне допустимого диапазона.")
                    continue

                month_start = df.loc[start-1, 'DATETIME'].month
                _, end = month_map.get(month_start, (None, None))
                if start > end:
                    print("❌ Строка выходит за пределы месяца.")
                    continue

                print(f"Стартовая строка {start} (обработка до конца месяца, строка {end})")
                return start, end
